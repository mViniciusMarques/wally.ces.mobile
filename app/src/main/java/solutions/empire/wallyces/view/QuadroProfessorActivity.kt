...
class QuadroProfessorActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        this.obterDados()
        this.obterProfessorBuscado()
        this.validarEntradaSemProfessor()
        this.QuadroProfessorService().execute()
    }
    ...
    inner class QuadroProfessorService : AsyncTask<Void, Void, String>(){
        ...
        private fun popularCamposComDadosRetornados(retornoParse: MutableList<ParseObject>) {
            if(retornoParse.isNotEmpty()) {
                ...
                Toast.makeText(applicationContext, "Professor não possui alterações", Toast.LENGTH_SHORT).show()
                startActivity(Intent(applicationContext, BuscarProfessorActivity::class.java))
            }
        }
